          ...
          return Buffer.isBuffer(text)
            ...
            : conn.sendMessage(
                jid,
                {
                  ...
                  contextInfo: global.adReply.contexInfo,
...
export function smsg(conn, m, hasParent) {
  ...
  if (m?.key?.remoteJid?.endsWith('@newsletter')) {
    console.log('📰 Newsletter message skipped in smsg');
    return null;
  }
  
  // Skip jika sender dari newsletter  
  if (m?.key?.participant?.endsWith('@newsletter')) {
    console.log('📰 Newsletter sender skipped in smsg');
    return null;
  }
...
export function serialize() {
  ...
  return Object.defineProperties(proto.WebMessageInfo.prototype, {
    ...
    isBaileys: {
      get() {
        // Skip newsletter
        if (this.key?.remoteJid?.endsWith('@newsletter')) {
          return false;
        }
        ...
        return (
          ...
          (this.id?.startsWith("3EB0") && this.id?.length === 22) || (this.id?.startsWith("SSA") && this.id?.length === 15) || 
          false
        );
      },
    },
    ...
    msg: {
      get() {
        ...
        const msg = this.message[this.mtype];
        
        // ==========================================
        // SAFE CHECK FOR CONTEXT INFO
        // ==========================================
        if (msg && typeof msg === 'object' && !msg.contextInfo) {
          msg.contextInfo = {};
        }
    ...
    quoted: {
      get() {
        ...
        // ==========================================
        // FILTER NEWSLETTER IN QUOTED
        // ==========================================
        
        // Skip jika quoted dari newsletter
        if (contextInfo?.remoteJid?.endsWith('@newsletter')) {
          return null;
        }
        
        // Skip jika participant dari newsletter
        if (contextInfo?.participant?.endsWith('@newsletter')) {
          return null;
        }
        
        // Safe check untuk quoted message
        ...
        return Object.defineProperties(
          JSON.parse(JSON.stringify(typeof q === "string" ? { text: q } : q)),
          {
            ...
            sender: {
      get() {
        const sender = this.conn?.decodeJid(
          (this.key?.fromMe && this.conn?.user.id) ||
            this.participant ||
            this.key.participant ||
            this.chat ||
    ...
    text: {
      get() {
        ...
        const text =
          ...
          msg?.message?.templateButtonReplyMessage?.selectedButtonId ||
          "";
        
        // Safe parsing for paramsJson
        let parsedId = "";
        try {
          if (text?.paramsJson && typeof text.paramsJson === 'string') {
            parsedId = JSON.parse(text.paramsJson)?.id || "";
          }
        } catch (e) {
          parsedId = "";
        }
