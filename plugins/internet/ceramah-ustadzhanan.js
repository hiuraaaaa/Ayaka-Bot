...
let handler = async(m, { text, conn, usedPrefix, command }) => {
   ...
   let response = await ttSearch('ceramah ustadz hanan hattaki')
   let { result } = response;
   ...
   let caption = `*Mohon Di Dengarkan*
    ✧ *Wm Creator:* ${author}`
    await conn.sendFile(m.chat, play, 'Lann4youofc.mp4', caption, m);
    } else {
    m.reply(eror)
    }
}
handler.help = ['ustadzhananhattaki','ustadzhanan']
handler.tags = ['ceramah']
handler.command = /^(ustadzhanan|ustadzhananhattaki)$/i
