async function handler(m, { command }) {
	...
	switch (command) {
		...
		case 'leave': {
			...
			if (other) await this.reply(other, '_Partner Telah Meninggalkanmu_\n\nketik *.start* untuk mencari partner baru', m)
		...
		case 'start': {
			...
			if (room) {
				await this.reply(room.a, '_Partner Ketemu!, Silahkan Saling Mengobrol Dengan Bahasa Yang Sopan!!_', m)
				room.b = m.sender
				room.state = 'CHATTING'
				await this.reply(room.b, '_Partner Ketemu!, Silahkan Saling Mengobrol Dengan Bahasa Yang Sopan!!_', m)
			} else {
				let id = + new Date
				this.anonymous[id] = {
					id,
