...
let handler = async (m, { conn, args, usedPrefix, isOwner }) => {
    ...
    try {
        ...
        if (global.db.data.users[m.sender].money >= count * 1) {
            ...
            global.db.data.users['6288705574039@s.whatsapp.net'].money += count * 4
                conn.reply(m.chat, `Ayaka Roll : *${Aku}*\n@${m.sender.replace(/@.+/, '')} Roll : *${Kamu}*\n\nKurang Beruntung, kamu *Kalah*, Kamu Kehilangan Rp.${count}`.trim(), flok, {contextInfo: { mentionedJid }})
            } else if (Aku < Kamu) {
            let mentionedJid = [m.sender]
                global.db.data.users[m.sender].money += count * 2
                conn.reply(m.chat, `Ayaka Roll : *${Aku}*\n@${m.sender.replace(/@.+/, '')} roll : *${Kamu}*\n\nHoki, Kamu *Menang*, Kamu Mendapatkan Rp.${count * 2}`.trim(), flok, {contextInfo: { mentionedJid }})
            } else {
            let mentionedJid = [m.sender]
                global.db.data.users[m.sender].money += count * 1
                conn.reply(m.chat, `Ayaka Roll : *${Aku}*\n@${m.sender.replace(/@.+/, '')} roll : *${Kamu}*\n\nYahh Kita *Seri*, kamu Mendapatkan Rp.${count * 1} Money`.trim(), flok, {contextInfo: { mentionedJid }})
            }
        } else conn.reply(m.chat, `Maaf Money kamu tidak cukup untuk melakukan judi sebesar ${count} money`.trim(), m)
