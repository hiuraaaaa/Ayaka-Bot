...
let handler = async (m, { conn }) => {
 ...
 │🧑🏻‍🍳 [ *Player* : @${m.sender.replace(/@.+/, '')} ]
 │📜 [ *Pesanan Ke* : ${wartegs[chatId][user].customerCount + 1}/5 ]
 │📝 [ *Makanan* : ${order} ]
⟣──────────⟢

Ketik .pesanan <makanan>`, flok, {contextInfo: { mentionedJid }});

        wartegs[chatId][user].timer = setTimeout(() => {
            if (wartegs[chatId][user].isOpen && wartegs[chatId][user].currentOrder) {
                conn.reply(chatId, `Info @${m.sender.replace(/@.+/, '')}, Warteg kamu tutup karena tidak melayani pelanggan tepat waktu.`, flok, {contextInfo: { mentionedJid }});
    ...
    if (command === '.warteg') {
        if (wartegs[chatId][user].isOpen) {
            conn.reply(chatId, `Hey @${m.sender.replace(/@.+/, '')}, Warteg kamu masih buka. Silakan layani pelanggan.`, flok, {contextInfo: { mentionedJid }});
            return;
        }
        ...
        if (currentTime - global.db.data.users[user].lastClosed < cooldown) {
            ...
            conn.reply(chatId, `@${m.sender.replace(/@.+/, '')}, Kamu sudah melayani pelanggan seharian, istirahatlah sejenak\n${Math.ceil(remainingTime / 60000)} menit lagi.`, flok, {contextInfo: { mentionedJid }});

            // Set a timeout to notify the user when the cooldown period is over
            setTimeout(() => {
                conn.reply(chatId, `@${m.sender.replace(/@.+/, '')}, Kamu sudah bisa buka warteg lagi! Ketik .warteg untuk memulai.`, flok, {contextInfo: { mentionedJid }});
            }, remainingTime);
            return;
        }
        ...
        if (!wartegs[chatId][user].isOpen) {
            conn.reply(chatId, `Aduh @${m.sender.replace(/@.+/, '')}, Warteg kamu belum buka. Ketik .warteg untuk mulai`, flok, {contextInfo: { mentionedJid }});
            return;
        }

        if (!wartegs[chatId][user].currentOrder) {
            conn.reply(chatId, `Sabar @${m.sender.replace(/@.+/, '')}, Tidak ada pelanggan saat ini. Tunggu pelanggan baru datang.`, null);
            return;
        }

        let userOrder = m.text.split(' ').slice(1).join(' ').trim().toLowerCase();
        if (!userOrder) {
            conn.reply(chatId, 'Kamu harus menyertakan nama makanan setelah .pesanan.', flok, {contextInfo: { mentionedJid }});
            return;
        }
        ...
        if (userOrder === wartegs[chatId][user].currentOrder) {
            rewardmoney = getRandomReward(100000, 2100000); // Random money reward between 100000 and 2100000
...
Tunggu 3 menit untuk pelanggan berikutnya`;
        ...
        conn.reply(chatId, message, flok, {contextInfo: { mentionedJid }});

        // Automatically close the warteg if 5 customers are served
        if (wartegs[chatId][user].customerCount >= 5) {
            conn.reply(chatId, `@${m.sender.replace(/@.+/, '')}, Warteg kamu tutup karena sudah malam setelah melayani 5 pelanggan. Kamu bisa membuka lagi dalam 30 menit.`, flok, {contextInfo: { mentionedJid }});
            clearInterval(wartegs[chatId][user].interval);
            clearTimeout(wartegs[chatId][user].timer);
            wartegs[chatId][user].isOpen = false;
            wartegs[chatId][user].currentOrder = null;
        ...
        if (!wartegs[chatId][user].isOpen) {
            conn.reply(chatId, 'Warteg kamu sudah tutup atau belum buka.', flok);
            return;
        }
        clearInterval(wartegs[chatId][user].interval);
        clearTimeout(wartegs[chatId][user].timer);
        ...
        wartegs[chatId][user].customerCount = 0;
        global.db.data.users[user].lastClosed = Date.now();
        conn.reply(chatId, 'Warteg ditutup.', flok);
    }
};
