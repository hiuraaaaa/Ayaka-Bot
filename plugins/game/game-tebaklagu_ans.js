...
export async function before(m, { conn }) {
  try {
    ...
    if (!/ketik.*hlagu/i.test(qtext)) return true
    if (/hlagu/i.test(mtext)) return true
    this.game = this.game || {}
    if (!(id in this.game))
    ...
    const answer = json.judul.toLowerCase().trim()
    ...
    console.error('❌ Error in tebaklagu:', e)
  }
  return true
}
