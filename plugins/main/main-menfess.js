let handler = async (m, { conn, text, usedPrefix, command }) => {
   ...
   // if (jid == m.sender) throw 'tidak bisa mengirim pesan memfess ke diri sendiri.'
    ...
    if (mf) return !0
    try {
        let mentionedJid = [jid + '@s.whatsapp.net']
    	let id = + new Date
        let txt = `Hai Kak *${conn.getName(jid)}*, kamu menerima pesan Menfess nih.
...
Mau balas pesan ini kak? bisa kak. kakak tinggal ketik pesan kakak nanti saya sampaikan ke ${name}.`.trim();
        await conn.reply(data.jid, txt, flok, {contextInfo: { mentionedJid }})
        .then(() => {
