const handler = async (m, {
    ...
    if (args.length < 1) return m.reply("Format pesan salah. Contoh penggunaan: .pushidgcdelay <delay>|<idgroup>|<pesan>");
    
    let [delayStr, groupId, text] = args.join(' ').split('|');
    
    if (!groupId || !text) return m.reply("Format pesan salah. Contoh penggunaan: .pushidgcdelay <delay>|<idgroup>|<pesan>");
...
handler.help = ['pushidgcdelay'];
handler.tags = ['pushkontak'];
handler.command = ['pushidgcdelay'];
handler.owner = true;
