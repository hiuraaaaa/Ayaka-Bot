...
async function scrapeAllSpecs(url) {
  try {
    ...
    const pAyakaEur = specs['Misc']?.['PAyaka'] || 'N/A';
    let pAyakas = { EUR: pAyakaEur };
    if (pAyakaEur !== 'N/A' && pAyakaEur.includes('EUR')) {
      const eurValue = parseFloat(pAyakaEur.match(/[\d.]+/)[0]);
    ...
    return { phoneName, specs, pAyakas, imageUrl, url };
  } catch (error) {
...
let handler = async (m, { conn, args, usedPrefix, command }) => {
  ...
  const { phoneName, specs, pAyakas, imageUrl, url } = result;
