...
const handler = async (m, { conn, args, usedPrefix, command }) => {
  ...
  try {
    ...
    const apiUrl = `${global.APIs.maelyn}/api/gemini/image?q=Ambil%20text%20digambar&url=${encodeURIComponent(imageUrl)}&apikey=${global.maelyn}`;
...
handler.tags = ['tools'];
handler.command = /^(copytext|ocr)$/i;
