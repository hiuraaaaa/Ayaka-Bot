...
let handler = async (m, {
    ...
    if (!validScales.includes(scale)) {
        return m.reply(
            `Nilai untuk hdr harus salah satu dari: ${validScales.join(", ")}.`,
        );
    }
    ...
    try {
        ...
        return m.reply(`Gagal melakukan hdr: ${error.message}`);
    }

    if (!response || !response.status) {
        return m.reply("Gagal melakukan hdr.");
    }

    conn.sendFile(m.chat, response.image, "hdr.jpg", "Ini dia, Kak!", m);
};

handler.help = ["hdv2"];
handler.tags = ["ai","premium"];
handler.command = /^(hdv2)$/i;
    ...
    try {
        return await new Promise((resolve, reject) => {
            ...
            if (!/(2|4|6|8|16)/.test(size.toString()))
                return reject("Invalid hdr size!");
