...
let handler = async (m, {
    ...
    if (args.length >= 1) {
        ...
        text = m.quoted.text
    } else return m.reply("Use *bardimg (text/media)*")
   ...
   if (mime && command === "bardimg") {
        ...
        let link = await uploadImage(media)
            try {
                ...
                let result = await translate(res, 'id');
...
async function AemtBardImg(query, url) {
    ...
    const bardRes = await fetch(`https://api.tioprm.eu.org/bardimg?url=${url}&text=${query}`, {
        method: "get",
        headers
    });
