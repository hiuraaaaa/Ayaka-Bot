...
const fkontak = {
    ...
    message: { conversation: `AI Couple v3 ✨` }
};

const STATIC_PROMPT = `Ultra-realistic cinematic photo of a real couple outdoors keeping both faces exactly identical to the uploaded photos for perfect authenticity. The man gently holds and kisses the waman's hand with his eyes closed, showing tenderness and deep affection. The woman gazes at him sottly with a faint loving smile, natural makeup, and long flowing hair Illuminated by warm sunlight. The scenie takes place in a real outdoor setting - soft golden-hour light, natural sun flares, shallow depth of field, and a blurred rocky background. High-resolution, natural skin texture, lifelike color tones, realistic environment lighting, cinematic framing, emotionally intimate atmosphere.`;
...
async function handleSecondImage(m, conn, sender) {
    await conn.sendMessage(m.chat, { text: `✅ Gambar kedua diterima.\n⏳ Memulai proses pembuatan foto couple v3, mohon tunggu... Ini mungkin memakan waktu 1-3 menit.` }, { quoted: fkontak });
...
handler.help = ['couple3', 'tocouple3'];
handler.tags = ['ai'];
handler.command = /^(couple3|tocouple3)$/i;
handler.limit = true;
