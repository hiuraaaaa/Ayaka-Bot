...
const fkontak = {
    ...
    message: { conversation: `AI Couple ✨` }
};

const STATIC_PROMPT = `Don't change their faces. Artistic fashion portrait of a young couple in luxurious thick knit white sweaters with thick textured yarn. They are framed in dramatic side-lighting, narrow sunlight cutting through their faces to create a striking chiaroscuro effect. The overall ambient background is minimalist, somber, and elegant, reminiscent of high-fashion editorial photography for luxury magazines, their heads are a little tilted.`;
...
async function handleSecondImage(m, conn, sender) {
    await conn.sendMessage(m.chat, { text: `✅ Gambar kedua diterima.\n⏳ Memulai proses pembuatan foto couple, mohon tunggu... Ini mungkin memakan waktu 1-3 menit.` }, { quoted: fkontak });
...
handler.help = ['couple', 'tocouple'];
handler.tags = ['ai'];
handler.command = /^(tocouple|couple)$/i;
handler.limit = true;
