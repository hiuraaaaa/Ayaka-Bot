...
const fkontak = {
    ...
    message: { conversation: `AI Couple v2 ✨` }
};

const STATIC_PROMPT = `A hyperrealistic, candid photo of an Indonesian couple on a sidewalk with a Spongebob mural wall in the background. The background is not blurry or bokeh. The man is wearing an oversized light blue t-shirt with a white graphic, white short jeans, and white sneakers. The woman is wearing a light blue t-shirt with a white graphic, a white baseball cap, a white asymmetrical short skirt, and white sneakers. The man is sitting on a black folding chair and the woman is standing beside him. Both are in a cool pose, changing their poses from the previous image, and looking directly at the camera. Streetwear and edgy vibes. Do not change their faces from the reference images.`;
const API_URL_BASE = "${global.faa}/faa/editfoto";
...
async function handleSecondImage(m, conn, sender) {
    await conn.sendMessage(m.chat, { text: `✅ Gambar kedua diterima.\n⏳ Memulai proses pembuatan foto couple v2, mohon tunggu... Ini mungkin memakan waktu 1-3 menit.` }, { quoted: fkontak });
...
handler.help = ['couple2', 'tocouple2'];
handler.tags = ['ai'];
handler.command = /^(couple2|tocouple2)$/i;
handler.limit = true;
