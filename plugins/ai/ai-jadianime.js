...
const fkontak = {
    ...
    message: { conversation: `AI To Anime ✨` }
};

const API_URL = `${global.faa}/faa/toanime?url=`;
...
const handler = async (m, { conn, usedPrefix, command }) => {
    ...
    try {
        ...
        await conn.sendMessage(m.chat, { text: '⏳ Sedang mengubah gambar Anda menjadi anime, mohon tunggu...' }, { quoted: fkontak });
        ...
        if (!resultBuffer || resultBuffer.length < 1000) { // Cek sederhana jika buffer valid
            throw new Error('API tidak mengembalikan gambar yang valid.');
        }

        await conn.sendMessage(m.chat, {
            image: resultBuffer,
            caption: 'Gambar To Anime telah jadi!✨'
        }, { quoted: fkontak });
...
handler.limit = true; // Atur sesuai kebutuhan Anda
