...
const fkontak = {
    ...
    message: { conversation: `AI Couple Bisokop 🎥` }
};

const STATIC_PROMPT = `Gunakan foto aku dan wanita sebagai referensi wajah (jangan diubah). Jadikan kedua foto tersebut sebagai objek utama, jangan mengubah wajah asli pria dan wanita pada foto, yang di ambil menggunakan kamera iPhone terbaru dengan jarak yang agak dekat dan dengan resolusi tinggi, jangan ada efek bokeh atau blur, bayangan lembut, kontras tinggi, memiliki rasio 9:16. mereka bergaya sedang berfoto selfie di depan cermin dalam sebuah lounge bioskop modern yang hangat dan terang khas indonesia. Pria berdiri santai dan memegang ponsel dengan tangan kanan untuk mengambil foto selfie dan sementara tangan kiri pria melingkari pinggangnya. Latar belakang menunjukkan dinding dengan poster film yang dibingkai, lantai berkarpet dengan pola geometris, pencahayaan dari lampu LED plafon, dan sebuah TV besar terlihat di kejauhan. Suasana tenang namun tetap hidup, kasual, bergaya pasangan muda asal Indonesia, dengan tema fesyen kartu remi yang unik dan playful.
Wajah kedua nya tampak terlihat jelas tidak tertutup objek apapun.`;
...
async function handleSecondImage(m, conn, sender) {
    await conn.sendMessage(m.chat, { text: `✅ Gambar kedua diterima.\n⏳ Memulai proses pembuatan foto couple bioskop, mohon tunggu... Ini mungkin memakan waktu 1-3 menit.` }, { quoted: fkontak });
...
handler.help = ['tobioskop', 'bioskop'];
handler.tags = ['ai'];
handler.command = /^(bioskop|tobioskop)$/i;
handler.limit = true;
